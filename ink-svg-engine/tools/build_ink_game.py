...
REPO = ROOT.parent
GAME = REPO/'street-fighter-6'
source = (GAME/'game.js').read_text(encoding='utf-8')
...
html = (GAME/'street-fighter-6.html').read_text(encoding='utf-8')
...
stages = {name:'data:image/svg+xml;base64,'+base64.b64encode((GAME/f'assets/stages/{name}.svg').read_bytes()).decode() for name in ['rooftop','courtyard']}
...
print('Built combat preview; upstream game.js sha256:', hashlib.sha256((GAME/'game.js').read_bytes()).hexdigest()[:16])
